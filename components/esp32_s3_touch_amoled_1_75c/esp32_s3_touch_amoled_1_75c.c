...
esp_err_t bsp_display_new(const bsp_display_config_t *config, esp_lcd_panel_handle_t *ret_panel, esp_lcd_panel_io_handle_t *ret_io)
{
    ...
    io_config.trans_queue_depth = 4; /* 降低队列深度以减少内部SRAM占用 */
...
{
    assert(cfg != NULL);
    /* buffer_height=10：每次 SPI flush 约 466*10*2=9.3KB bounce buffer，
     * 远小于 WiFi 运行后内部 SRAM 可用连续块，避免 setup_dma_priv_buffer 失败。
     * max_transfer_sz 与 buffer_height 保持一致。 */
    #define LVGL_TRANSFER_BUF_LINES  10
